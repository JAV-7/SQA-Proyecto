...
# ── Manipulación de sys.path (debe ir antes de imports locales) ───────────────
...
from sklearn.linear_model import ElasticNetCV, LassoCV, LinearRegression, RidgeCV  # noqa: E402
from sklearn.metrics import mean_squared_error, r2_score  # noqa: E402
from sklearn.model_selection import train_test_split  # noqa: E402

# ── Local (requiere sys.path configurado) ─────────────────────────────────────
    ...
    return pd.DataFrame(
        {
            ...
            "Category": rng.choice(
                ["electronics", "furniture", "clothing", "toys", "groceries"], n
            ),
...
def _make_preprocessed_df(n=80, seed=0):
    ...
    for c in [
        "Category___electronics",
        "Category___furniture",
        "Category___clothing",
        "Category___toys",
        "Store___1",
        "Store___2",
...
class TestDataClean:
    ...
    def test_clean_data_returns_dataframe(self, raw_df, tmp_path):
        with (
            patch.object(_dc, "REPORTS_DIR", tmp_path),
    ...
    def test_get_initial_insights_writes_file(self, raw_df, tmp_path):
        with (
            patch.object(_dc, "REPORTS_DIR", tmp_path),
            patch.object(
                _dc, "INITIAL_INSIGHTS_PATH", tmp_path / "insights.txt"
            ),
        ):
...
class TestLinealRegression:
    ...
    def test_print_model_info(self, preprocessed_df, capsys):
        ...
        pipe = Pipeline([("dropper", ct), ("linreg", LinearRegression())]).fit(X, y)
...
class TestRandomForest:
    ...
    def test_missing_columns_detection(self, preprocessed_df):
        ...
        df_new = preprocessed_df.drop(columns=["objetivo", "PC1"], errors="ignore")
...
class TestRegularization:
    ...
    def test_lasso(self, train_test):
        X_tr, _, y_tr, _ = train_test
        lasso = LassoCV(
            alphas=np.logspace(-2, 2, 10), cv=3, max_iter=1000, random_state=42
        ).fit(X_tr, y_tr)
    ...
    def test_build_metrics_table(self, train_test):
        ...
        models = (
            ...
            ElasticNetCV(
                alphas=[0.1], l1_ratio=[0.5], cv=3, max_iter=500
            ).fit(X_tr, y_tr),
        )
    ...
    def test_build_and_save_coefficients(self, train_test, tmp_path):
        ...
        with patch.object(_reg, "FILES_DIR", tmp_path):
            _reg._build_and_save_coefficients(X_tr.columns.tolist(), models)
    ...
    def test_build_and_save_pca_betas(self, train_test, tmp_path):
        ...
        with patch.object(_reg, "FILES_DIR", tmp_path):
            _reg._build_and_save_pca_betas(X_tr.columns.tolist(), reg, elastic)
    ...
    def test_save_best_bundle_creates_zip(self, train_test, tmp_path):
        ...
        with patch.object(_reg, "REGULARIZATION_BUNDLE_DIR", tmp_path):
            _reg._save_best_bundle(
                "Elastic Net", 0.75, models, X_tr.columns.tolist(), df
            )
...
# Importar módulos de Common_Functions
try:
...
class TestCommonFunctionsColumns:
    ...
    def test_get_columns_numeric_types(self):
        if not _CF_AVAILABLE:
            pytest.skip("Common_Functions no disponible")
        df = pd.DataFrame(
            {"int_col": [1, 2], "float_col": [1.1, 2.2], "str_col": ["a", "b"]}
        )
        num, cat = _cf_cols.get_columns(df)
...
class TestDataCleanExtra:
    ...
    def test_plot_categorical_columns_runs(self, tmp_path):
        df = pd.DataFrame(
            {
                "Category": ["electronics", "furniture", "clothing", "toys"],
                "Store": ["s1", "s2", "s1", "s2"],
            }
        )
        with patch.object(_dc, "GRAPHICS_DIR", tmp_path):
    ...
    def test_ensure_output_dirs(self, tmp_path):
        with (
            patch.object(_dc, "REPORTS_DIR", tmp_path / "r"),
    ...
    def test_save_clean_data_creates_unknown_csv(self, tmp_path):
        df = _make_raw_df()
        df["Demand Forecast"] = (
            np.random.default_rng(0).integers(10, 400, len(df)).astype(float)
        )
        with patch.object(_dc, "FILES_DIR", tmp_path):
            _dc.save_clean_data(df)
        assert (
            tmp_path / "retail_store_inventory_produccion_unknown.csv"
        ).exists()
...
class TestLinealRegressionExtra:
    ...
    def test_save_artifacts(self, preprocessed_df, tmp_path):
        ...
        with (
            patch.object(_lr, "MODEL_DIR", tmp_path),
...
class TestRandomForestExtra:
    ...
    def test_save_artifacts_rf(self, preprocessed_df, tmp_path):
        ...
        progress = tqdm(total=2, disable=True)
        with (
            patch.object(_rf, "MODEL_PATH", tmp_path / "model.pkl"),
            patch.object(
                _rf, "EXPECTED_COLUMNS_PATH", tmp_path / "cols.json"
            ),
            patch.object(
                _rf, "FEATURE_IMPORTANCE_PATH", tmp_path / "imp.csv"
            ),
...
class TestEnsureOutputDirs:
    ...
    def test_lr_ensure_output_dirs(self, tmp_path):
        with (
            patch.object(_lr, "REPORTS_DIR", tmp_path / "r"),
    ...
    def test_me_ensure_output_dirs(self, tmp_path):
        with (
            patch.object(_me, "REPORTS_DIR", tmp_path / "r"),
    ...
    def test_reg_ensure_output_dirs(self, tmp_path):
        with (
            patch.object(_reg, "REGULARIZATION_FILES_DIR", tmp_path / "rf"),
...
class TestLoadFunctionsWithMocks:
    ...
    def test_load_models(self, tmp_path, preprocessed_df):
        ...
        fake_lin.mkdir()
        fake_rf.mkdir()
        with patch.object(_me, "_load_models", return_value=(m, m)):
    ...
    def test_train_models_returns_four(self, train_test):
        ...
        with patch.object(
            ...
            wraps=lambda X, y: (
                ...
                LassoCV(alphas=alphas, cv=3, max_iter=500).fit(X, y),
                ElasticNetCV(
                    alphas=alphas, l1_ratio=[0.5], cv=3, max_iter=500
                ).fit(X, y),
            ),
        ):
...
class TestModelEvaluationOrchestrator:
    ...
    def test_model_evaluation_success(self, tmp_path, preprocessed_df):
        ...
        joblib.dump(m, path_rf)
        with (
            patch.object(_me, "PREPROCESSING_DIR", tmp_path),
            patch.object(_me, "REPORTS_DIR", tmp_path),
            patch.object(_me, "GRAPHICS_DIR", tmp_path),
    ...
    def test_model_evaluation_file_not_found(self):
        with patch.object(
            _me, "_load_test_data", side_effect=FileNotFoundError("no file")
        ):
            result = _me.model_evaluation()
